...
def nn_caler(img, net, layer_names):
	...
	for nfbp in range(len(NON_FUNC_BOXES_POINTS)):	
		...
		cv2.rectangle(img, (x, y), (x + int(rect_scale*w), y + int(rect_scale*h)), (255, 0, 0), 2)# Отрисовка ректангла по координатам центра коробки, полученными из функций
		cv2.circle(img, (x, y), 25, (0,255,0), -1)# Отрисовка координат внутри ректангла производится относительно этой точки
